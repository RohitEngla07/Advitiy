...
% 1 41783U 16059A   18067.11436293 +.00000038 +00000-0 +16791-4 0  9994
% 2 41783 098.1306 130.2437 0034615 054.5309 305.9115 14.62904422077188
...
tic
orbInc=  98.2049;
meanMo = 14.63229411;%revolutions per day
SGPdragp1 = 4.6090e-05;  % from ---First time derivative of mean motion(rad/min^2)
SGPdragp2 =0;     % Second time derivative of mean motion (rad/min^3
SGP4dragp = 8.8930; %Bstar
n0 = 2*pi*meanMo/1440; % Mean motion (rad/min) 
    e0 = 0.0030; % Eccentricity (0.0<=e0>=1.0)   2)
    M0 = 96.7138*pi/180; %5.339; % Mean anomaly (rad)    4)
    w0 = 263.1090*pi/180; %054.5309; % Argument of perigee (rad)   5)
    Ohm0 = 329.2442; %rsc
    revNo = 19;

% orbInc=  098.1306;
% meanMo = 14.62904422077188;%revolutions per day
%  SGPdragp1 = 3.8000e-07;  % from ---First time derivative of mean motion(rad/min^2)
% SGPdragp2 =0;     % Second time derivative of mean motion (rad/min^3
% SGP4dragp = 0.1679; %Bstar
% n0 = 2*pi*meanMo/1440; % Mean motion (rad/min) 
%     e0 = 0.0035; % Eccentricity (0.0<=e0>=1.0)   2)
      i0 = pi*orbInc/180; % Inclination (rad)      3)
%     M0 = 5.339; % Mean anomaly (rad)    4)
%     w0 = 054.5309; % Argument of perigee (rad)   5)
%     Ohm0 = 130.2437; %rsc
      dn0 = 2*2*pi*SGPdragp1/(1440^2); % First time derivative of mean motion(rad/min^2)
      ddn0 = 6*2*pi*SGPdragp2/(1440^3); % Second time derivative of mean motion (rad/min^3)
      Bstar = SGP4dragp; % SGP4 type drag coefficient
